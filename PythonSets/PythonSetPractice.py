...
print("_"*40)
##################################################
#set1 = {4, 7, 9, 11, 25, 34, 56}
set1 = {1, 7, 11, 13, 17, 23}
prime_output = []

for ele in set1:
    #print(ele)
    prime_ele = 0
    for val in range(2, ele):
        if ele%val == 0:
            #prime_ele += 1
            prime_ele = prime_ele + 1


    if prime_ele ==0:
        prime_output.append(ele)
if len(set1) == len(prime_output):
    print("all the elements are prime number")
else:
    print("all the elements are not prime number")
